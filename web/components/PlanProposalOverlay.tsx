...
import type { PlanProposal, ProposedPlanTask } from '@/lib/plan-generator'
...
export function PlanProposalOverlay({ planId, loading, error, proposal, onClose, onRetry, onAccepted }: Props) {
  const [rootGoal, setRootGoal] = useState('')
  const [rootGoalSet, setRootGoalSet] = useState(false)
  const [children, setChildren] = useState<(ProposedPlanTask & { accepted: boolean })[]>([])
  ...
  async function handleAccept() {
    ...
    setAcceptError(null)
    try {
      const res = await fetch(`/api/plans/${planId}/generate-plan/accept`, {
        method: 'POST',
