...
import { usePlanWebSocket } from '@/lib/use-plan-ws'
...
import type { PlanRow, TreeNode, TreeStats, Event, AgentSession } from '@/lib/db'
import type { PlanProposal } from '@/lib/plan-generator'
...
export function PlanView({ project, tree, stats, events, agentSession }: Props) {
  ...
  const [genProposal, setGenProposal] = useState<PlanProposal | null>(null)
  ...
  async function handleGenerate() {
    ...
    setGenProposal(null)
    try {
      const res = await fetch(`/api/plans/${project.id}/generate-plan`, { method: 'POST' })
  ...
  async function handleCreateTask(e: React.FormEvent) {
    ...
    setNewTaskError(null)
    try {
      const res = await fetch(`/api/plans/${project.id}/tasks`, {
        method: 'POST',
  ...
  async function handleSendPrompt(e: React.FormEvent) {
    ...
    try {
      if (!currentSession) {
        ...
        const runRes = await fetch(`/api/plans/${project.id}/agent/run`, {
          method: 'POST',
          headers: { 'Content-Type': 'application/json' },
      ...
      const promptRes = await fetch(`/api/plans/${project.id}/agent/prompt`, {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
  ...
  async function handleArchive() {
    setActing(true)
    await fetch(`/api/plans/${project.id}/archive`, { method: 'POST' })
    router.refresh()
    router.push('/')
  }
  ...
  async function handleUnarchive() {
    setActing(true)
    await fetch(`/api/plans/${project.id}/restore`, { method: 'POST' })
    router.refresh()
    router.push('/')
  }
  ...
  async function handleDelete() {
    ...
    setActing(true)
    await fetch(`/api/plans/${project.id}`, { method: 'DELETE' })
    router.refresh()
    router.push('/')
  }
  ...
  return (
    <div className="h-screen flex flex-col bg-white">
      ...
      <header className="border-b flex items-center gap-4 px-4 py-2 flex-shrink-0">
        ...
        <SettingsButton />
        <AgentBadge session={currentSession} planId={project.id} />
      </header>
