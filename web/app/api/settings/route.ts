...
import { z } from 'zod'
import { getConfig, saveConfig } from '@/lib/conductor-config'
...
export async function POST(req: NextRequest) {
  try {
    ...
    const { anthropic_api_key, anthropic_auth_token } = parsed.data
    if (!anthropic_api_key && !anthropic_auth_token) return err('Provide at least one of anthropic_api_key or anthropic_auth_token')
