...
describe('POST /api/projects/:id/tasks', () => {
  it('creates root task with ID "1"', async () => {
    ...
    const res = await createTask(project.id, { goal: 'Root task', plan: ['step 1'], status: 'active' })
  ...
  it('creates child task with correct tree-address ID', async () => {
    ...
    const res = await createTask(project.id, { goal: 'Child A', plan: ['step'], parent_id: '1' })
    const { body } = await json(res)
    expect(body.id).toBe('1.1')

    const res2 = await createTask(project.id, { goal: 'Child B', plan: ['step'], parent_id: '1' })
  ...
  it('rejects depends_on referencing non-sibling', async () => {
    const project = await createProject()
    await createTask(project.id, { goal: 'Root', plan: ['step'] })
    await createTask(project.id, { goal: 'Child 1', plan: ['step'], parent_id: '1' })
...
describe('GET /api/projects/:id/tasks', () => {
  it('returns full tree structure', async () => {
    const project = await createProject()
    await createTask(project.id, { goal: 'Root', plan: ['s'] })
    await createTask(project.id, { goal: 'Child', plan: ['s'], parent_id: '1' })
...
describe('GET /api/projects/:id/tasks/:taskId', () => {
  it('returns task with parent, siblings, children, stats', async () => {
    const project = await createProject()
    await createTask(project.id, { goal: 'Root', plan: ['s'] })
    await createTask(project.id, { goal: 'Child A', plan: ['s'], parent_id: '1' })
    await createTask(project.id, { goal: 'Child B', plan: ['s'], parent_id: '1' })
...
describe('PATCH /api/projects/:id/tasks/:taskId', () => {
  it('updates goal', async () => {
    const project = await createProject()
    await createTask(project.id, { goal: 'Original', plan: ['s'] })
  ...
  it('updates notes', async () => {
    const project = await createProject()
    await createTask(project.id, { goal: 'Task', plan: ['s'] })
    const res = await PATCHTask(
...
describe('POST /api/projects/:id/tasks/:taskId/status', () => {
  it('changes status to completed', async () => {
    const project = await createProject()
    await createTask(project.id, { goal: 'Root', plan: ['s'], status: 'active' })
  ...
  it('requires reason when abandoning', async () => {
    const project = await createProject()
    await createTask(project.id, { goal: 'Task', plan: ['s'], status: 'active' })
  ...
  it('blocks activation when dependency is not completed', async () => {
    const project = await createProject()
    await createTask(project.id, { goal: 'Root', plan: ['s'] })
    await createTask(project.id, { goal: 'A', plan: ['s'], parent_id: '1', status: 'pending' })
    await createTask(project.id, { goal: 'B', plan: ['s'], parent_id: '1', status: 'pending' })
  ...
  it('includes warning when completing task with unresolved children', async () => {
    const project = await createProject()
    await createTask(project.id, { goal: 'Root', plan: ['s'], status: 'active' })
    await createTask(project.id, { goal: 'Child', plan: ['s'], parent_id: '1', status: 'pending' })
